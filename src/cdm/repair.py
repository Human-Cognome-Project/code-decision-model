...
def candidate_symbol(candidate: str) -> str:
    ...
    if match is None:
        raise ValueError(f"candidate does not begin with a function signature: {first!r}")
    return match.group(1)


def extract_python(text: str) -> str:
    """Extract one fenced Python block or return stripped raw output."""
    stripped = text.strip()
    fence = chr(96) * 3
...
def verify_repair(example: DecisionExample, generated: str) -> RepairVerification:
    """Require the generated caller to be AST-equivalent to the exact target repair."""
    if example.task not in _SUPPORTED_TASKS:
    ...
    if actual_dump != expected_dump:
        target = candidate_symbol(example.candidates[example.answer_index])
        candidate_names = {candidate_symbol(c) for c in example.candidates}
...
def feedback_for(verification: RepairVerification) -> str:
    ...
    messages = {
        "invalid_python": "The repair is not a single valid Python function. Return a complete function.",
        "wrong_function": "The caller identity changed. Preserve the original caller and revise only the repair.",
        "placeholder_remaining": "The masked call target is still present. Replace it with one candidate.",
        "wrong_target": "The selected call target violates the repository constraint. Reconsider the candidates.",
        "unrelated_change": "The repair changed code outside the masked call target. Make only the required call-target edit.",
...
def build_repair_prompt(
    ...
    *,
    ...
    """Build a repair prompt with an optional fallible decision-model recommendation."""
    if example.task not in _SUPPORTED_TASKS:
    ...
    candidates = (nl + nl).join(
        f"Candidate {i + 1}:{nl}{candidate}"
        for i, candidate in enumerate(example.candidates)
    )
    sections = [
        "Repair the Python caller below.",
        (
            "Replace __CALL_TARGET__ with the correct candidate while preserving "
            "every other operation. Return only the complete repaired Python function."
        ),
