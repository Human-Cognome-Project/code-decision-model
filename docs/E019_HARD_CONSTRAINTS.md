...
constraints. E019 adds a minimal interface so deterministic validators can
veto or filter candidates after the discriminative scorer has produced logits.
...
- Constraints are deterministic for the supplied decision and any machine-checkable facts captured by the constraint object.
...
oracles, test outcomes, etc.) can plug into the same `Constraint` protocol by
capturing precomputed deterministic facts in the constraint object.
