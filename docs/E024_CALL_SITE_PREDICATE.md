...
E024 adds the first constraint derived from real machine-checkable facts and
...
The census script checks empirical soundness on the current machine-labelled corpora: no labelled target may be vetoed in those data. The test suite repeats the same check over this repository snapshot. This is a regression guard, not a universal proof that every possible Python call is covered.

## Census on the contribution snapshot
