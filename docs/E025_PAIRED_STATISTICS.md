...
E025 adds both as `cdm.corrective`. It contains no model and changes no harness.
...
arms so the pairing is preserved. The conditional correction-turn statistic
...
interval = cluster_bootstrap_paired(examples, outcomes, statistic="success_rate_delta")
